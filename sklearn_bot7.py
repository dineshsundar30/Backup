...
class MT5TradingBot:
    ...
    def check_trading_signal(self, df):
        ...
        current_pattern = {
            ...
            'price_level': float(last_row['close'])
        }
        
        # Check if pattern is in failed patterns
        pattern_str = json.dumps(current_pattern)
        if any(pattern["pattern_data"] == pattern_str for pattern in failed_patterns):
            self.logger.info("Pattern matched with previous failed pattern - skipping trade")
            return 0, None
    ...
    def execute_trade(self, signal, pattern):
        try:
            ...
            self.logger.info(f"Executing trade: {self.fixed_lot} lots")
            ...
            if result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                trade_data = {
                    ...
                    'supertrend_value': float(pattern['supertrend_direction']),
                    'macd_value': float(pattern['macd_cross']),
                    'signal_value': signal
                }
    ...
    def check_account_viability(self):
        ...
        if account_info.balance < self.min_balance:
            self.logger.warning(f"Account balance (${account_info.balance:.2f}) below minimum requirement (${self.min_balance:.2f})")
            return False
        return True
