...
def main(page: ft.Page):
    ...
    def add_contact_view():
        ...
        for i, c in enumerate(contacts):
            duplicate = c['phone'] in seen_numbers
            seen_numbers.add(c['phone'])
            ...
            contact_row = ft.Row([
                ...
                ft.Column([
                    ...
                    ft.Text(c['phone'], size=12, color=ft.Colors.RED if duplicate else ft.Colors.GREY_600)
                ], expand=True),
    ...
    def bulk_send_messages(e=None):
        ...
        name_field.bgcolor = ft.Colors.RED_100 if not name_field.value.strip() else ft.Colors.WHITE
        message_field.bgcolor = ft.Colors.RED_100 if not message_field.value.strip() else ft.Colors.WHITE
        page.update()

        if not name_field.value.strip() or not message_field.value.strip():
            page.snack_bar = ft.SnackBar(
                content=ft.Text("Business name and message cannot be empty!"),
        ...
        if duplicates_found:
            page.snack_bar = ft.SnackBar(
                content=ft.Text("Duplicate numbers found in selected contacts!"),
                bgcolor=ft.Colors.BLUE
            )
            page.snack_bar.open = True
            page.update()
        ...
        def send():
            ...
            for i, index in enumerate(selected_list):
                ...
                phone = contact['phone']
                name = contact['name']
                rephrased_msg = rephrase_message(name, f"{name_field.value.strip()} - {message_field.value.strip()}")
    ...
    splash = ft.View(
        ...
        controls=[
            ft.Container(
                ...
                content=ft.Text("🤖 Smart Broadcast Messenger\n                                        - by 'DK'", size=28, weight="bold", color=ft.Colors.WHITE)
...
ft.app(target=splash_screen, view=ft.AppView.FLET_APP)
