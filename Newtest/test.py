...
# Trading parameters
SYMBOL = "XAUUSDm"  # Change to your desired forex pair
...
def place_trade(symbol, lot_size, prediction, current_price, order_type):
    ...
    print(
        f"[TRADE] {order_type_str} Order placed - Ticket #{result.order}, Price: {current_price}, Prediction: {prediction:.5f}")
    return True
            ...
            try:
                ...
                if active_positions < MAX_ACTIVE_TRADES:
                    ...
                    if price_diff_percent > 0.01:  # Minimum 0.01% difference
                        order_type_str = 'BUY' if order_type == mt5.ORDER_TYPE_BUY else 'SELL'
                        print(
                            f"[SIGNAL] {order_type_str} - Current: {last_close:.5f}, Predicted: {prediction:.5f}, Diff: {price_diff_percent:.3f}%")
